...
const Movie = props => {
    ...
    return (
        <div className={'highlight_movie' + ((props.movie.main_trailer === null) ? ' highlight_movie--no_trailer' : '') + ((props.index === 1) ? ' highlight_movie--left' : '') }>
            {   smallDevice && 
                <Link to={`/movie/${props.movie.name.replace(" ", "-")}-MV-${props.movie.id}`}><h3>{props.movie.name}</h3></Link>
            }
            <div className={'movie__graphic' + ((iframeClicked && props.movie.main_trailer) ? ' movie__graphic--remove-border-radius' : '')
                ...
                {props.movie.main_trailer ?
                    ...
                    />
                :
                    <Link to={`/movie/${props.movie.name.replace(" ", "-")}-MV-${props.movie.id}`}>
            ...
            <div className='movie__description'>
                ...
                { props.movie.screenings.length !== 0 ? 
                    <Link className='button shadow-tiny' to={`/movie/${props.movie.name.replace(" ", "-")}-MV-${props.movie.id}#tickets`}>Tickets</Link>
                :
                    <Link className='button shadow-tiny' to={`/movie/${props.movie.name.replace(" ", "-")}-MV-${props.movie.id}`}>See details</Link>
                }
            </div>
        </div>
    )
