                ...
                props.screenings.map(screening => {
                    ...
                    return (
                        <div key={screening.id} className='calendar__screening'>
                            <h4>{`${hour}:${minute}`}</h4>
                            <Link className='calendar__screening__img' to={`/movie/${screening.slug}-${screening.type}-${screening.show_id}`}>
                                <img src={screening.image ? screening.image : "/static/images/logo.png"} alt="show image"/>
                            </Link>
                            <Link className='calendar__screening__name' to={`/movie/${screening.slug}-${screening.type}-${screening.show_id}`}><h3>{screening.name}</h3></Link>
                        </div>
                    )
                })
            }
